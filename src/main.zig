...
pub fn main() !void {
    ...
    // Rendering!
    const sphere = scene.Hittable{ .sphere = scene.Sphere{ .radius = 0.5, .origin = zm.Vec3{ .data = .{ 0, 0, -1 } } } };
    const pseudo_plane = scene.Hittable{ .sphere = scene.Sphere{ .radius = 100, .origin = zm.Vec3{ .data = .{ 0, -100.5, -1 } } } };
    ...
    for (0..cam.img_height) |j| {
        for (0..cam.img_width) |i| {
            const pixel_center = cam.pixel00_loc.add(cam.pixel_delta_u.scale(@floatFromInt(i))).add(cam.pixel_delta_v.scale(@floatFromInt(j)));
